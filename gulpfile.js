...
function stage() {
    const streams = filesToCopy.map(file => {
        if (fs.existsSync(file)) {
            return gulp.src(file).pipe(gulp.dest('package/discord-rich-presence'));
        }
    }).filter(s => s != undefined);
    return merge([
        ...streams,
        gulp.src('dist/**/*').pipe(gulp.dest('package/discord-rich-presence/dist')),
        gulp.src('static/**/*').pipe(gulp.dest('package/discord-rich-presence/static')),
    ]);
}
